...
const Carousel: FC<PropsWithChildren<CarouselProps>> = ({
  ...
  const carouselCount = useMemo(() => Children.count(children) || 0, [children]);
  ...
  useLayoutEffect(() => {
    ...
    const rangedCurrentIndex = Math.min(Math.max(0, currentIndex), carouselCount - 1);
  ...
  useEffect(() => {
    if (carouselCount > 1 && autoplay) {
      carouselTimer.current = setTimeout(() => {
        setActiveIndex(prevActiveIndex => (prevActiveIndex + 1) % carouselCount);
      }, duration);
    }
  ...
  return (
    <div className={clns(styles.carousel, customClassName)} style={{ ...(style ?? {}) }} data-testid="carousel">
      <SlickCarousel speed={speed} activeIndex={activeIndex} easing={easing}>
        {children}
      </SlickCarousel>
      {showIndicator && (
        <Indicator count={carouselCount} activeIndex={activeIndex} duration={duration} onClick={handleClickIndicator} />
      )}
    </div>
  );
};
