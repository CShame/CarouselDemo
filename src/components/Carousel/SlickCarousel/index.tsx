...
const SlickCarouselSlideItem: FC<PropsWithChildren<{ style?: CSSProperties }>> = ({ style, children }) => {
  return (
    <div className={styles.slick_carousel_slide_item} style={{ ...(style ?? {}) }}>
      {children}
    </div>
  );
};

const SlickCarousel: FC<PropsWithChildren<SlickCarouselProps>> = memo(({ activeIndex, speed, easing, children }) => {
  ...
  const slickCarouselItems = useMemo(() => {
    ...
    if (Array.isArray(children) && children.length) {
      return Children.map(children, child => (
        <SlickCarouselSlideItem style={slideItemStyle}>{child}</SlickCarouselSlideItem>
      ));
    }
    return <SlickCarouselSlideItem style={slideItemStyle}>{children}</SlickCarouselSlideItem>;
  ...
  return (
    <div className={styles.slick_carousel} ref={slickCarouselRef}>
      <div className={styles.slick_carousel_slide_wrapper} style={slickCarouselSlideWrapperStyle}>
        {slickCarouselItems}
      </div>
    </div>
  );
